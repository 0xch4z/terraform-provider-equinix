...
func getAbstractVirtualCircuit(vc *metalv1.VirtualCircuit) abstractVirtualCircuit {
	if vc.VlanVirtualCircuit != nil {
		return vc.VlanVirtualCircuit
	}

	return vc.VrfVirtualCircuit
}
...
func parseConnection(
	...
	*speed = types.StringValue(connSpeed)
	...
	if conn.GetType() == metalv1.INTERCONNECTIONTYPE_SHARED {
		...
		vc := getAbstractVirtualCircuit(&conn.Ports[0].VirtualCircuits[0])
		*projectID = types.StringPointerValue(vc.GetProject().Id)
...
func parseConnectionPorts(ctx context.Context, cps []metalv1.InterconnectionPort) (fwtypes.ListNestedObjectValueOf[PortModel], diag.Diagnostics) {
	...
	for _, p := range cps {
		...
		for i, vc := range p.VirtualCircuits {
			portVcIDs[i] = types.StringValue(getAbstractVirtualCircuit(&vc).GetId())
		}
